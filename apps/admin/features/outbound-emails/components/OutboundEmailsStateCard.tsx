type OutboundEmailsStateCardProps = {
  title: string;
  message: string;
...
export function OutboundEmailsStateCard({
  title,
  message,
  tone,
  actionLabel,
  onAction,
}: OutboundEmailsStateCardProps) {
  const classNameByTone: Record<OutboundEmailsStateCardProps['tone'], string> =
    {
      error: 'border-[#FCA5A5]',
  ...
  const titleClassNameByTone: Record<
    OutboundEmailsStateCardProps['tone'],
    string
  > = {
    error: 'text-[#B91C1C]',
    neutral: 'text-[#111827]',
